...
// ---- Auto viewBox: fit all authored geometry --------------------------------
...
function renderConnectionPath(conn, index) {
  ...
  return `        <path ${focusEdgeAttrs(conn.from, conn.to, conn.label, index, conn.id)} data-composition-points="${routePointsValue(routed.points)}" d="${routed.d}" class="${cls}" stroke-width="${strokeWidth}" marker-end="url(#${marker})"/>`;
}
...
function renderComponent(c) {
  ...
  const passport = { kind: c.semantic_kind || c.type, sublabel: c.sublabel, tag: c.tag, context: componentContext(c) };
